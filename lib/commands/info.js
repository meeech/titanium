	...
	async.parallel([
		...
		function (next) {
			if (process.platform === 'darwin' && (types.all || types.ios)) {
				appc.ios.detect(function (info) {
					...
					result.iOSDevices = info.iOSDevices;
		...
		if (cli.argv.output == 'json') {
			...
			if ((types.all || types.titanium) && data.titanium) {
				...
				try {
					cliVersion = appc.pkginfo.package(module).version;
				} catch (e) {}
				try {
					nodeAppcVer = appc.pkginfo.package(path.resolve(path.join(module.filename, '..', '..', '..', 'node_modules', 'node-appc'))).version;
				} catch (e) {}
				
				sections['Titanium CLI'] = [
					['CLI Version', cliVersion || 'unknown'],
					['node-appc Version', nodeAppcVer || 'unknown']
				];
				...
				Object.keys(data.titanium).forEach(function (ver) {
					...
					try {
						nodeAppcVer = appc.pkginfo.package(path.join(x.path, 'node_modules', 'node-appc')).version;
					} catch (e) {}
					...
					titanium[ver] = [
						...
						['node-appc Version', nodeAppcVer || 'n/a']
					];
				});
			}
			...
			if (process.platform == 'darwin' && (types.all || types.ios)) {
				...
				pp = sections['Connected iOS Devices'] = {};
				if (data.iOSDevices.length) {
					data.iOSDevices.forEach(function (device) {
						pp[device.name] = [
							['Type', device.deviceClass],
							['iOS Version', device.productVersion + ' [' + device.buildVersion + ']'],
							['Color', device.deviceColor],
							['Product Type', device.productType],
							['Device UDID', device.id],
							['Serial #', device.serialNumber],
							['CPU Architecture', device.cpuArchitecture],
